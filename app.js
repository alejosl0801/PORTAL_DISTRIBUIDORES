...
var _passChangeObligatorio=false;
...
function generarPassAleatoria(){
  ...
  var result="Pyro-",len=4+Math.floor(Math.random()*2);
...
function hacerLogin(){
  ...
  if(d.esAdmin){mostrar("s-admin");renderAdmin();}
    ...
    if(!localStorage.getItem(key)){
      iniciarTutorial(); // cerrarTutorial() revisará si requiere cambio de pass
    } else if(USER.requiereCambioPass){
      setTimeout(function(){mostrarCambioPassObligatorio();},500);
    }
  }
}
...
function cerrarTutorial(){
  ...
  if(USER&&USER.requiereCambioPass){setTimeout(function(){mostrarCambioPassObligatorio();},400);}
}

// ════════════════════ CAMBIO DE CONTRASEÑA ════════════════════
function mostrarCambioPassObligatorio(){
  _passChangeObligatorio=true;
  document.getElementById("cp-nueva").value="";
  document.getElementById("cp-repetir").value="";
  document.getElementById("cp-err").textContent="";
  document.getElementById("cp-titulo").textContent="Crea tu contraseña personal";
  document.getElementById("cp-desc").textContent="Por seguridad debes elegir una contraseña propia antes de continuar. No puedes omitir este paso.";
  document.getElementById("cp-cancelar").style.display="none";
  abrir("modal-cambio-pass");
}
function mostrarCambioPassOpcional(){
  _passChangeObligatorio=false;
  document.getElementById("cp-nueva").value="";
  document.getElementById("cp-repetir").value="";
  document.getElementById("cp-err").textContent="";
  document.getElementById("cp-titulo").textContent="Cambiar contraseña";
  document.getElementById("cp-desc").textContent="Elige una nueva contraseña para tu cuenta.";
  document.getElementById("cp-cancelar").style.display="";
  abrir("modal-cambio-pass");
}
function guardarNuevaPass(){
  var nueva=document.getElementById("cp-nueva").value.trim();
  var rep=document.getElementById("cp-repetir").value.trim();
  ...
  if(nueva===USER.pass){errEl.textContent="La nueva contraseña debe ser diferente a la actual.";return;}
  if(nueva!==rep){errEl.textContent="Las contraseñas no coinciden. Vuelve a intentarlo.";return;}
  guardarCambioPass(USER.ruc,nueva);
  _passChangeObligatorio=false;
...
function renderAdmDist(){
  ...
  document.getElementById("adm-dist-lista").innerHTML=lista.length?lista.map(function(d){
    ...
    return '<div class="card"><div class="card-b">'+
      ...
      '<div style="font-size:12px;margin-top:6px;background:var(--g1);border-radius:8px;padding:6px 10px;display:flex;justify-content:space-between;align-items:center;gap:8px">'+
        '<span>🔑 <b style="color:var(--azul)">'+d.pass+'</b>'+
        (d.requiereCambioPass?'<span style="font-size:10px;color:#e69900;margin-left:6px">· Sin cambiar</span>':'')+'</span>'+
        '<button class="btn btn-sm btn-s" style="font-size:11px;padding:3px 8px;flex-shrink:0" onclick="copiarCredenciales(\''+rucSafe+'\')">📋 Copiar</button>'+
      '</div>'+
...
function abrirNuevoDist(){
  document.getElementById("nd-pass").value=generarPassAleatoria();
...
function guardarNuevoDist(){
  ...
  var nd={ruc:ruc,tipoDoc:tipoDoc,razon:r,pass:pw,tel:tel,correo:co,entrega:{habilitada:ent,montoMin:min},_nuevo:true,requiereCambioPass:true};
  ...
  cerrarModal("modal-nuevo-dist");renderAdmDist();
  // Mostrar credenciales al admin
  document.getElementById("nd-cred-ruc").textContent=ruc;
  document.getElementById("nd-cred-pass").textContent=pw;
  abrir("modal-cred-nuevo");
  ["nd-razon","nd-empresa","nd-ruc","nd-tel","nd-correo","nd-dir"].forEach(function(x){document.getElementById(x).value="";});
  document.getElementById("nd-pass").value=generarPassAleatoria();
...
// Persistencia de cambios de contraseña (distribuidores base, no _nuevo)
...
function copiarAlPortapapeles(txt){
  if(navigator.clipboard&&navigator.clipboard.writeText){
    navigator.clipboard.writeText(txt).then(function(){toast("📋 Copiado al portapapeles");}).catch(function(){_copiarFallback(txt);});
...
function _copiarFallback(txt){
  ...
  catch(e){toast("Copia manual: "+txt);}
...
function copiarNuevoCred(){
  var ruc=document.getElementById("nd-cred-ruc").textContent;
  var pass=document.getElementById("nd-cred-pass").textContent;
  copiarAlPortapapeles("Usuario: "+ruc+" / Contraseña: "+pass);
}

// Cerrar modal al tocar fondo (bloqueado si hay cambio de pass obligatorio)
document.addEventListener("click",function(e){
  if(e.target.classList.contains("ov")){
    if(_passChangeObligatorio&&e.target.id==="modal-cambio-pass")return;
